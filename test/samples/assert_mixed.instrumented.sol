...
contract AssertMixed {
    ...
    function main(uint[] memory n) public {
        ...
        unchecked {
            if (!(n.length < 10)) {
                emit __ScribbleUtilsLib__34.AssertionFailed("00720:063:00 2: ");
                assert(false);
            }
        }
    ...
    function _original_AssertMixed_main(uint[] memory n) private {
        ...
        unchecked {
            ...
            if (!(_v.forall_0)) {
                emit __ScribbleUtilsLib__34.AssertionFailed("01361:063:00 3: ");
                assert(false);
            }
        }
        for (uint i = 0; i < n.length; i++) {
            unchecked {
                if (!(n[i] > 2)) {
                    emit __ScribbleUtilsLib__34.AssertionFailed("01606:063:00 4: ");
                    assert(false);
                }
            }
    ...
    function __scribble_AssertMixed_check_state_invariants_internal() internal {
        unchecked {
            if (!(sum < 200)) {
                emit __ScribbleUtilsLib__34.AssertionFailed("02018:063:00 0: ");
                assert(false);
            }
        }
    }
    ...
    function AssertMixed_sum_uint256_plus_assign(uint256 ARG0) internal returns (uint256 RET0) {
        ...
        unchecked {
            if (!((sum - _v.old_0) > 3)) {
                emit __ScribbleUtilsLib__34.AssertionFailed("02840:063:00 1: ");
                assert(false);
            }
        }
    }
