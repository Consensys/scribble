...
contract ERC721 {
    ...
    function balanceOf(address owner) virtual public returns (uint256 RET_0) {
        ...
        unchecked {
            if (!(owner != address(0))) {
                emit __ScribbleUtilsLib__826.AssertionFailed("02533:101:00 1: NFTs cannot be owned by the 0 address");
                assert(false);
            }
        }
    }
    ...
    function ownerOf(uint256 tokenId) virtual public returns (address RET_0) {
        ...
        unchecked {
            if (!(RET_0 != address(0))) {
                emit __ScribbleUtilsLib__826.AssertionFailed("03118:101:00 2: NFTs cannot be owned by the 0 address");
                assert(false);
            }
            if (!(_original_ERC721_balanceOf(RET_0) > 0)) {
                emit __ScribbleUtilsLib__826.AssertionFailed("03342:105:00 3: ownerOf() should not contradict balanceOf");
                assert(false);
            }
        }
    }
    ...
    function approve(address to, uint256 tokenId) virtual public {
        ...
        unchecked {
            if (!((msg.sender == _original_ERC721_ownerOf(tokenId)) || isApprovedForAll(_original_ERC721_ownerOf(tokenId), msg.sender))) {
                emit __ScribbleUtilsLib__826.AssertionFailed("05051:110:00 16: Sender must be properly authorized to approve");
                assert(false);
            }
            if (!(getApproved(tokenId) == to)) {
                emit __ScribbleUtilsLib__826.AssertionFailed("05273:088:00 17: Approve works correctly");
                assert(false);
            }
            if (!(_original_ERC721_ownerOf(tokenId) == _v.old_0)) {
                emit __ScribbleUtilsLib__826.AssertionFailed("05492:097:00 18: Approve doesn't change ownership");
                assert(false);
            }
        }
    }
    ...
    function setApprovalForAll(address operator, bool approved) virtual public {
        ...
        unchecked {
            if (!(approved == isApprovedForAll(msg.sender, operator))) {
                emit __ScribbleUtilsLib__826.AssertionFailed("06531:099:00 19: setApprovalForAll worked correctly");
                assert(false);
            }
        }
    }
    ...
    function transferFrom(address from, address to, uint256 tokenId) virtual public {
        ...
        unchecked {
            if (!(to != address(0))) {
                emit __ScribbleUtilsLib__826.AssertionFailed("07580:093:00 12: Cannot transfer to 0 address");
                assert(false);
            }
            if (!(_v.old_1 == from)) {
                emit __ScribbleUtilsLib__826.AssertionFailed("07775:095:00 13: from must be the current owner");
                assert(false);
            }
            if (!(((msg.sender == _v.old_2) || _v.old_3) || (_v.old_4 == msg.sender))) {
                emit __ScribbleUtilsLib__826.AssertionFailed("08022:111:00 14: Sender must be properly authorized to transfer");
                assert(false);
            }
            if (!(_original_ERC721_ownerOf(tokenId) == to)) {
                emit __ScribbleUtilsLib__826.AssertionFailed("08258:080:00 15: Transfer worked");
                assert(false);
            }
        }
    }
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId) virtual public {
        ...
        unchecked {
            if (!(to != address(0))) {
                emit __ScribbleUtilsLib__826.AssertionFailed("09185:092:00 8: Cannot transfer to 0 address");
                assert(false);
            }
            if (!(_v.old_5 == from)) {
                emit __ScribbleUtilsLib__826.AssertionFailed("09379:094:00 9: from must be the current owner");
                assert(false);
            }
            if (!(((msg.sender == _v.old_6) || _v.old_7) || (_v.old_8 == msg.sender))) {
                emit __ScribbleUtilsLib__826.AssertionFailed("09625:111:00 10: Sender must be properly authorized to transfer");
                assert(false);
            }
            if (!(_original_ERC721_ownerOf(tokenId) == to)) {
                emit __ScribbleUtilsLib__826.AssertionFailed("09861:080:00 11: Transfer worked");
                assert(false);
            }
        }
    }
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) virtual public {
        ...
        unchecked {
            if (!(to != address(0))) {
                emit __ScribbleUtilsLib__826.AssertionFailed("10723:092:00 4: Cannot transfer to 0 address");
                assert(false);
            }
            if (!(_v.old_9 == from)) {
                emit __ScribbleUtilsLib__826.AssertionFailed("10917:094:00 5: from must be the current owner");
                assert(false);
            }
            if (!(((msg.sender == _v.old_10) || _v.old_11) || (_v.old_12 == msg.sender))) {
                emit __ScribbleUtilsLib__826.AssertionFailed("11166:110:00 6: Sender must be properly authorized to transfer");
                assert(false);
            }
            if (!(_original_ERC721_ownerOf(tokenId) == to)) {
                emit __ScribbleUtilsLib__826.AssertionFailed("11401:079:00 7: Transfer worked");
                assert(false);
            }
        }
    }
