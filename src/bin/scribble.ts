...
if ("version" in options) {
     ...
     */
    if (options["disarm"]) {
        if (fse.existsSync(metaDataFile)) {
            const metaData = fse.readJsonSync(metaDataFile, { encoding: "utf-8" });

            for (const file of metaData.originalSourceList) {
                originalFiles.add(file);
            }

            for (const file of metaData.instrSourceList) {
                if (file !== "--") {
                    instrumentationFiles.add(file);
                }
            }
            ...
            console.log(instrumentationFiles);

            process.exit(1);
        } else {
            // Can not throw here due to previous instrumentation will remain in projects
        }
    ...
    if (options["instrumentation-metadata-file"] !== undefined) {
        ...
        writeOut(JSON.stringify(metadata, undefined, 2), options["instrumentation-metadata-file"]);
    }
}
